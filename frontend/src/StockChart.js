...
const StockChart = ({ getCommodityHistoricalData }) => {
  const [commodityTickerSymbol, setCommodityTickerSymbol] = useState(useParams().ticker_symbol);
  const [selectedMetrics, setSelectedMetrics] = useState(['close']); // Initially display "close" prices
  const [startIndex, setStartIndex] = useState(0); // Start index for the slider
  const [endIndex, setEndIndex] = useState(); // End index for the slider
  const [historicalData, setHistoricalData] = useState([]);
  ...
  useEffect(() => {
    async function fetchHistoricalData() {
      const data = await getCommodityHistoricalData(commodityTickerSymbol);
      setHistoricalData(data);
      setStartIndex(0); // Default to the first data point
      setEndIndex(data.length - 1); // Default to the last data point
    }
    fetchHistoricalData();
  }, [commodityTickerSymbol, getCommodityHistoricalData]);
  ...
  // Date range filtering based on slider indices
  const filteredData = historicalData.slice(startIndex, endIndex + 1);
    ...
    <div>
      ...
      <div>
        <label>
          ...
          />
          <span>{new Date(historicalData[startIndex].date).toLocaleDateString()}</span>
        </label>
        <br />
        <label>
          End Date:
          ...
          />
          <span>{new Date(historicalData[endIndex].date).toLocaleDateString()}</span>
        </label>
      </div>
